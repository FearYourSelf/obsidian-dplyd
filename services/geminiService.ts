...
export const streamChatResponse = async (
  ...
  tier: ModelTier,
  enableThinking: boolean, // Deprecated argument, ignored
  ...
  // NOTE: ThinkingConfig removed to prevent 503/429 errors.
...
export const generateSpeech = async (text: string, voiceName: string = 'Zephyr', accent: string = 'australian', retries = 3): Promise<AudioBuffer | null> => {
    // Basic text cleanup
    let cleanText = text
    ...
    switch (accent) {
        case 'american': voicePrompt += " Speak with a standard American accent."; break;
        case 'british': voicePrompt += " Speak with a refined British accent."; break;
        case 'italian': voicePrompt += " Speak with a heavy, stereotypical Italian accent. Mix in Italian words enthusiastically. Be very expressive."; break;
        case 'australian': default: voicePrompt += " Speak with a warm, bright, and energetic Australian accent."; break;
    }
    if (voiceName === 'Fenrir') voicePrompt += " Maintain a deep, authoritative tone.";
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            const isOverloaded = e.status === 503 || (e.message && e.message.includes('overloaded'));
            if (isOverloaded && attempt < retries) {
                // Exponential backoff: 1s, 2s, 4s...
                const delay = 1000 * Math.pow(2, attempt);
                console.warn(`TTS Model Overloaded (503). Retrying in ${delay}ms... (Attempt ${attempt + 1}/${retries})`);
                await wait(delay);
                continue;
            }
            console.error("TTS Final Error:", e);
            return null;
        }
    }
    return null;
